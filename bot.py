...
CALC_BUTTON_TEXT = "📦 Рассчитать пакеты EVA"
...
async def post_in_chat(context: ContextTypes.DEFAULT_TYPE, chat_id: int, is_channel: bool = False) -> None:
    text = (
        "📦 *Калькулятор пакетов-слайдеров EVA*\n\n"
        "Рассчитайте стоимость пакетов EVA за несколько секунд.\n"
...
async def start_private(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        "Добро пожаловать! 🎉\n\n"
        "Нажмите кнопку ниже, чтобы открыть калькулятор пакетов EVA.\n"
